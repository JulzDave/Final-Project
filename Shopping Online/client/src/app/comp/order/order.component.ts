...
export class OrderComponent implements OnInit {
  ...
  creditCardValidation(): void {
    ...
    if (isMasterCard.test(this.orderDetails.controls.creditCard.value)) {
      ...
      isAmericanExpress.test(this.orderDetails.controls.creditCard.value)
    ) {
      this.matIcon = "check";
      document.getElementById("matIcon").style.color = "green";
      document.getElementById("matIcon").style.left = "135px";
      ...
      isDinersOrDiscover.test(this.orderDetails.controls.creditCard.value)
    ) {
      this.matIcon = "check";
      document.getElementById("matIcon").style.color = "green";
      document.getElementById("matIcon").style.left = "135px";
  ...
  return(): void {
    (
      document.getElementsByTagName("body") as HTMLCollectionOf<HTMLBodyElement>
  ...
  autofiller(ev: any): any {
    if (ev.target.placeholder == "City") {
      ...
      return this.orderDetails.controls.street.setValue(
        this.user.address.street
      );
    } else if (ev.target.placeholder == "House Num") {
      return this.orderDetails.controls.houseNumber.setValue(
        this.user.address.houseNum
      );
    } else if (ev.target.placeholder == "Apt.") {
      return this.orderDetails.controls.aptNumber.setValue(
        this.user.address.apt
      );
    }
  }
  ...
  orderAction(): void {
    ...
    this.submitted = true;
    if (
      this.f4.deliveryDate.value &&
      this.f4.expirationDate.value &&
      this.orderDetails.valid &&
      this.matIcon === "check"
    ) {
      (
      ...
      this.shopService
        ...
        .subscribe((data) => {
          ...
          if (data.cartID) {
            ...
            this.shopService
              .lockCartProd({ cartID: data.cartID, generated: data.generated })
              .subscribe((data) => {
                this.shopService
                  .lockCart({
                    userID: this.user._id,
                    generated: this.orderGeneration,
                  })
                  .subscribe((data2) => {
                    this.success = true;
  ...
  computeOrderDeliveryOverload(): void {
    ...
    if (this.totalAmountOfOrders > 0) {
        this.allOrderDates = data
          .map((item) => item.scheduled.split(":")[0].substr(0, 15))
          .sort()
          .reverse();
        var i: number = 0;

        while (i < this.totalAmountOfOrders) {
          var duplicatedDateInstance = this.allOrderDates.filter(
            (item) => item == this.allOrderDates.filter((item) => item)[i]
          );
        ...
        this.allDupedDates = this.allDupedDates.map((item) =>
  ...
  downloadPDF(): void {
    ...
    var headers2: any = createHeaders2([
      "Date",
      "Card_number",
      "Card_type",
      "Order_price",
    ]);
    var headers3: any = createHeaders3([
      "First_name",
      "Last_name",
      "Purchase_order_num",
